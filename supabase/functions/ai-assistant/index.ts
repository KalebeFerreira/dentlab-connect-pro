...
serve(async (req) => {
  ...
  try {
    ...
    // Detecta se é service_role (n8n)
    const isServiceRole = authHeader.includes("service_role") || authHeader.length > 100;
    ...
    // Só valida usuário se NÃO for service_role
    if (!isServiceRole) {
      ...
      // 🔒 Mantém verificação de plano só aqui
    ...
    const totalRevenue = financial
      .filter((f) => f.transaction_type === "receipt")
...
${
  orders
    .slice(0, 5)
    .map((o) => `- ${o.work_type} para ${o.patient_name} (${o.clinic_name}) - Status: ${o.status}`)
