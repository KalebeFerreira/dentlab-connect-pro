...
import { Download, FileText, FileSpreadsheet, Send, User, Users } from "lucide-react";
...
export const ProductionExport = ({ employees, workRecords, periodFilter, labName = "Laboratório" }: ProductionExportProps) => {
  ...
  const [exportEmployeeId, setExportEmployeeId] = useState<string | null>(null);
 
  const getFilteredData = (employeeId?: string | null): EmployeeReport[] => {
    const now = new Date();
    let startDate: Date;
    let endDate: Date;
  ...
  const exportToPDF = async (employeeId?: string | null) => {
    try {
      const data = getFilteredData(employeeId);
      if (data.length === 0) {
      ...
      const isIndividual = employeeId && data.length === 1;
      const title = isIndividual 
        ? `Relatório de Produção - ${data[0].employee.name}`
      ...
      for (const report of data) {
        ...
        // Table header with patient and color
        doc.text("Paciente", 14, y);
        doc.text("Tipo", 50, y);
        doc.text("Cor", 85, y);
        doc.text("Início", 105, y);
        doc.text("Prazo", 125, y);
        ...
        for (const record of report.records) {
          ...
          doc.text(patientName.length > 18 ? patientName.substring(0, 18) + "..." : patientName, 14, y);
          const workType = WORK_TYPES[record.work_type] || record.work_type;
          doc.text(workType.length > 15 ? workType.substring(0, 15) + "..." : workType, 50, y);
          doc.text(record.color || "-", 85, y);
          doc.text(format(new Date(record.start_date), "dd/MM/yy"), 105, y);
          doc.text(record.deadline ? format(new Date(record.deadline), "dd/MM/yy") : "-", 125, y);
          doc.text(record.status === "finished" ? "Finalizado" : "Em Andamento", 145, y);
          doc.text(record.value ? record.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : "-", 175, y);
          y += 4;
        }
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(9);
        doc.text(`Subtotal: ${report.records.length} trabalhos | ${report.finishedCount} finalizados | ${report.totalValue.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`, 14, y);
        doc.setFontSize(11);
        y += 12;
      }

      const fileName = isIndividual 
        ? `relatorio-${data[0].employee.name.toLowerCase().replace(/\s+/g, '-')}-${format(new Date(), "yyyy-MM-dd")}.pdf`
        : `relatorio-producao-${format(new Date(), "yyyy-MM-dd")}.pdf`;
  ...
  const exportToExcel = async (employeeId?: string | null) => {
    try {
      const data = getFilteredData(employeeId);
      if (data.length === 0) {
        toast.error("Nenhum dado para exportar");
      ...
      detailsSheet.columns = [
        ...
        { header: "Tipo de Trabalho", key: "workType", width: 22 },
      ...
      a.href = url;
      const fileName = isIndividual 
        ? `relatorio-${data[0].employee.name.toLowerCase().replace(/\s+/g, '-')}-${format(new Date(), "yyyy-MM-dd")}.xlsx`
        : `relatorio-producao-${format(new Date(), "yyyy-MM-dd")}.xlsx`;
      a.download = fileName;
      a.click();
  ...
  const exportToWord = async (employeeId?: string | null) => {
    try {
      const data = getFilteredData(employeeId);
      if (data.length === 0) {
        toast.error("Nenhum dado para exportar");
      ...
      for (const report of data) {
        ...
        html += `
            <tr class="subtotal">
              <td colspan="6">Subtotal: ${report.records.length} trabalhos (${report.finishedCount} finalizados)</td>
      ...
      a.href = url;
      const fileName = isIndividual 
        ? `relatorio-${data[0].employee.name.toLowerCase().replace(/\s+/g, '-')}-${format(new Date(), "yyyy-MM-dd")}.doc`
        : `relatorio-producao-${format(new Date(), "yyyy-MM-dd")}.doc`;
      a.download = fileName;
      a.click();
      URL.revokeObjectURL(url);
            ...
            {activeEmployees.length > 0 && (
              <>
                ...
                <DropdownMenuSub>
                  ...
                  <DropdownMenuSubContent>
                    {activeEmployees.map(emp => (
                      <DropdownMenuItem key={emp.id} onClick={() => exportToPDF(emp.id)}>
                        {emp.name}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuSubContent>
                ...
                <DropdownMenuSub>
                  ...
                  <DropdownMenuSubContent>
                    {activeEmployees.map(emp => (
                      <DropdownMenuItem key={emp.id} onClick={() => exportToWord(emp.id)}>
                        {emp.name}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuSubContent>
                ...
                <DropdownMenuSub>
                  ...
                  <DropdownMenuSubContent>
                    {activeEmployees.map(emp => (
                      <DropdownMenuItem key={emp.id} onClick={() => exportToExcel(emp.id)}>
                        {emp.name}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuSubContent>
